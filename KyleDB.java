    ...
    public static void runDB() {
        ...
        while(!(endflag)) {
            try {
                ...
                if (user_input.toLowerCase().equals("help".toLowerCase())) {
                ...
                } else if(user_input.toLowerCase().equals("quit".toLowerCase())) {
                    endflag = true;
                    ...
                    System.out.printf("\n\n>> ");
                } else if(user_input.toLowerCase().startsWith("db.CS557.query".toLowerCase())) {
                    System.out.printf("Query\n\n>> ");
                } else if(user_input.toLowerCase().startsWith("db.CS557.max".toLowerCase())) {
                ...
                } else if(user_input.toLowerCase().startsWith("db.CS557.avg".toLowerCase())) {
                    int sum = 0;
                    int count = 0;
                    String field = user_input.substring(13, user_input.length() - 1);
                    for (int i = 0; i < storage.size(); i++) {
                    ...
                    System.out.printf("%d\n\n>> ",sum/count);
                } else if(user_input.toLowerCase().startsWith("db.CS557.sum".toLowerCase())) {
                    int sum = 0;
                    String field = user_input.substring(13, user_input.length() - 1);
                    for (int i = 0; i < storage.size(); i++) {
                        for (int j = 0; j < storage.get(i).size(); j++) {
                    ...
                    System.out.printf("%d\n\n>> ",sum);
                } else if(user_input.toLowerCase().startsWith("db.CS557.cartprod".toLowerCase())) {
                    ...
                    System.out.printf("\n>> ");
                } else if(user_input.toLowerCase().startsWith("db.CS557.sort".toLowerCase())) {
                    String field = user_input.substring(14, user_input.length() - 1);
                    System.out.printf("Sort on %s\n\n>> ",field);
                } else if(user_input.toLowerCase().startsWith("db.CS557.delete".toLowerCase())) {
...
class Document {
    private KVpair[] doc_array;
    private int length;

    public Document()
    {
        doc_array = new KVpair[100];
        length = 0;
    }

    public void addPair(KVpair kvpair)
    {
        if (doc_array.length == length) {
            KVpair[] newdoc_array = new KVpair[2 * length];
            for (int i = 0; i < length; i++) { newdoc_array[i] = doc_array[i]; }
            doc_array = newdoc_array;
        }

        doc_array[length++] = kvpair;
    }

    public KVpair get(int i) { return doc_array[i]; }
...
class KVpair {
    ...
    public int compareTo(KVpair otherPair) {
        if(this.key.toLowerCase().equals(otherPair.getKey().toLowerCase())) {
            return 1;
        }
        return 0;
    }
